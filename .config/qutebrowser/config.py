...
## Custom function to close tabs when pressing back at the end of history
@cmdutils.register()
@cmdutils.argument('tab', value=cmdutils.Value.cur_tab)
@cmdutils.argument('count', value=cmdutils.Value.count)
def back_or_close(tab, count=1):
    """Go back or close tab, if at beginning of history."""
    if tab.history.can_go_back():
        try:
            tab.history.back(count)
        except browsertab.WebTabError as e:
            raise cmdutils.CommandError(e)
    else:
        tab.window_close_requested.emit()
...
config.bind('<back>', 'back-or-close')
